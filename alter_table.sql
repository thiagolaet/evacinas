...
ALTER TABLE APLICA
	ADD CONSTRAINT pk_ap PRIMARY KEY(cod_fr_ap, cod_lo_fr_ap),
    ...
    ON UPDATE CASCADE ON DELETE RESTRICT,
    ADD CONSTRAINT fk_fr_ap FOREIGN KEY (cod_fr_ap, cod_lo_fr_ap) REFERENCES FRASCO(cod_fr,cod_lo_fr)
    ON UPDATE CASCADE ON DELETE RESTRICT;
...
CREATE TRIGGER update_usado_fr AFTER INSERT 
ON APLICA 
FOR EACH ROW 
UPDATE FRASCO SET usado_fr = 1 
WHERE FRASCO.cod_fr = NEW.cod_fr_ap AND FRASCO.cod_lo_fr = NEW.cod_lo_fr_ap;
